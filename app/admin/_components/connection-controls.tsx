...
function ConnectionForm({ provider }: { provider: SyncProvider }) {
  ...
  const fields = provider === "binance"
    ...
    : [{ key: "token", label: "Flex Web Service token", secret: true }, { key: "queryId", label: "Activity Flex Query ID", secret: false }];
  return (
    <form key={formKey} onSubmit={(event) => { if (pending) event.preventDefault(); else onSubmit(event); }} className="flex flex-col gap-6">
      ...
      <FormFooter state={state} pending={pending}>{pending ? "Connecting…" : "Save & sync"}</FormFooter>
    </form>
  );
}

export function CredentialExpiryButton({ provider, expiresOn }: { provider: SyncProvider; expiresOn: string | null }) {
...
function CredentialExpiryForm({ provider, expiresOn }: { provider: SyncProvider; expiresOn: string | null }) {
...
export function ConnectAccountButton({ provider, connected = false }: { provider: SyncProvider; connected?: boolean }) {
  return (
...
export function ConnectionSettings({ provider, enabled, included, hasSnapshot }: {
  provider: SyncProvider; enabled: boolean; included: boolean; hasSnapshot: boolean;
}) {
  const [pending, startTransition] = useTransition();
