...
export function transferEligible(a: ImportedEntry, b: ImportedEntry): boolean {
...
export function transferSuggestions(entries: ImportedEntry[]): [ImportedEntry, ImportedEntry][] {
  const candidates = new Map<string, ImportedEntry[]>();
  ...
  return entries.filter((a) => a.amount < 0 && candidates.get(a.id)?.length === 1)
    .flatMap((a) => { const b = candidates.get(a.id)![0]; return candidates.get(b.id)?.length === 1 ? [[a, b] as [ImportedEntry, ImportedEntry]] : []; });
}
